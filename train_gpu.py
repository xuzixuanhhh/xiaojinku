"""DX-PINN GPU training — scaled model + v6 strategy — target SOTA @ -10dB"""
...
print('\nPhase 2: Joint training (200 epochs, GPU)')
...
# EMA weight tracking for stable evaluation (eliminates oscillation in metrics)
ema_decay = 0.999
...
def update_ema(model, ema, decay):
    with torch.no_grad():
        for k, v in model.state_dict().items():
            if k in ema: ema[k] = decay * ema[k] + (1 - decay) * v.detach()

def swap_ema(model, ema):
    orig = {k: v.clone() for k, v in model.state_dict().items()}
    model.load_state_dict(ema); return orig

def restore(model, orig):
...
for ep in range(200):
    ...
    for batch_data, batch_label, batch_t in train_loader:
        ...
        update_ema(model, ema_state, ema_decay)  # smooth weights
    sched.step()
    train_acc = calculate_metrics(yt, yp)[0]
    if ep % 3 == 0:
        orig_w = swap_ema(model, ema_state)  # eval with stable EMA weights
        model.eval()
        c = eval_at(20); m6 = eval_at(-6); m10 = eval_at(-10)
        restore(model, orig_w)  # restore training weights
        print(f'E{ep+1:3d}: TA={train_acc:.4f} Clean={c:.4f} -6dB={m6:.4f} -10dB={m10:.4f}')
        score = c * 0.3 + m6 * 0.3 + m10 * 0.4
        if score > best_score:
            best_score = score
            orig_w2 = swap_ema(model, ema_state)
            torch.save(model.state_dict(), os.path.join(SAVE_PATH, 'dx_pinn_best.pth'))
            restore(model, orig_w2); patience = 0
        else: patience += 1
