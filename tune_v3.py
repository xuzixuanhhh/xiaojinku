...
sep = '=' * 60
print(sep)
print('DX-PINN v3: BEAM + RGAF + SE Attention Screening')
print('Params: ~189K (7% of original)')
print(sep)
sys.stdout.flush()
...
for lr in LRS:
    ...
    if mean_m10 > best_mean:
        best_mean = mean_m10; best_lr = lr

print(f'\nBest LR: {best_lr} (-10dB={best_mean:.4f})')
sys.stdout.flush()

print(f'\n{sep}')
print(f'DX-PINN v3: 10-Run Validation (lr={best_lr})')
print(sep)
sys.stdout.flush()
...
for run in range(10):
    ...
    c, m4, m10, best = train_v3(run, best_lr)
    rc.append(c); r4.append(m4); r10.append(m10); r10b.append(best)
    print(f'Clean={c:.4f} -4dB={m4:.4f} -10dB={m10:.4f} (best={best:.4f})')

print(f'\n{sep}')
print('DX-PINN v3 (100ep) vs Baselines -- CWRU 1HP')
print(sep)
print(f'{"Model":<22} {"Params":>8} {"-10dB":>16} {"-4dB":>12} {"Clean":>10}')
print(f'{"-"*68}')
print(f'{"v3 BEAM+RGAF 100ep":<22} {"189K":>8} {np.mean(r10)*100:>7.2f}%+/-{np.std(r10)*100:.2f}% {np.mean(r4)*100:>7.2f}% {np.mean(rc)*100:>7.2f}%')
print(f'{"Lite 100ep":<22} {"777K":>8} {"69.03%+/-3.62%":>16} {"94.47%":>12} {"99.13%":>10}')
print(f'{"Original 100ep":<22} {"2.64M":>8} {"66.70%+/-6.59%":>16} {"93.62%":>12} {"99.71%":>10}')
print(f'{"Original 200ep":<22} {"2.64M":>8} {"76.76%+/-1.97%":>16} {"98.41%":>12} {"99.84%":>10}')
