"""DX-PINN v7: Residual SUD + wider classifier for -10dB"""
...
device = DEVICE
train_loader, _, _ = get_dataloader(0, train_snr_range=(-12, 2), train_noise=True, val_noise=True, test_noise=True)
_, _, td, _, _, tlbl = load_dataset(0)
model = DX_PINN().to(device)
print(f'Params: {sum(p.numel() for p in model.parameters()):,}')

# Phase 1
print('\nPhase 1: Residual denoiser pretrain @ -10dB')
...
for ep in range(15):
    ...
    for batch_data, _, _ in train_loader:
        ...
        sig_pow = (batch_data ** 2).mean(dim=1, keepdim=True)
        noise = torch.randn_like(batch_data) * torch.sqrt(sig_pow / 10.0)
        noisy = batch_data + noise
        denoised, noise_pred, est_snr = model.denoiser(noisy)
        L = torch.nn.functional.mse_loss(denoised, batch_data) + 0.5 * torch.nn.functional.mse_loss(noise_pred, noise)
...
for ep in range(120):
    ...
    for batch_data, batch_label, batch_t in train_loader:
        ...
        loss = L_cls + 3.0 * L_eap + 0.15 * L_recon
