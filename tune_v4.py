...
def train_v4(seed, lr):
    ...
    for ep in range(100):
        m.train()
        if ep < 5:
            for pg in o2.param_groups: pg['lr'] = lr * (ep + 1) / 5
        for bd, bl, bt in tl:
            bd, bl, bt = bd.to(device), bl.to(device), bt.to(device).requires_grad_(True)
            o2.zero_grad()
            out = m(bd, bt)
    ...
    m.eval()
    return quick_ev(m, td, tlbl, 10), quick_ev(m, td, tlbl, -4), quick_ev(m, td, tlbl, -10), best_m10
...
sep = '=' * 60
print(sep)
print('DX-PINN v4: ECA + Selective Kernel + Dynamic Gate')
print('~733K params | HIDDEN_DIM=96')
print(sep)
sys.stdout.flush()
...
for lr in LRS:
    ...
    sys.stdout.flush()
    if mean_m10 > best_mean: best_mean = mean_m10; best_lr = lr

print(f'\nBest LR: {best_lr} (-10dB={best_mean:.4f})')
sys.stdout.flush()

print(f'\n{sep}')
print(f'DX-PINN v4: 10-Run Validation (lr={best_lr})')
print(sep)
sys.stdout.flush()
...
for run in range(10):
    ...
    c, m4, m10, best = train_v4(run, best_lr)
    rc.append(c); r4.append(m4); r10.append(m10); r10b.append(best)
    print(f'Clean={c:.4f} -4dB={m4:.4f} -10dB={m10:.4f} (best={best:.4f})')

print(f'\n{sep}')
print('DX-PINN v4 vs Baselines -- CWRU 1HP, 100 epochs')
print(sep)
print(f'{"Model":<22} {"Params":>8} {"-10dB":>16} {"-4dB":>12} {"Clean":>10}')
print(f'{"-"*68}')
print(f'{"v4 ECA+SK+DyGate":<22} {"733K":>8} {np.mean(r10)*100:>7.2f}%+/-{np.std(r10)*100:.2f}% {np.mean(r4)*100:>7.2f}% {np.mean(rc)*100:>7.2f}%')
print(f'{"Lite (best so far)":<22} {"777K":>8} {"69.03%+/-3.62%":>16} {"94.47%":>12} {"99.13%":>10}')
print(f'{"Original (baseline)":<22} {"2.64M":>8} {"66.70%+/-6.59%":>16} {"93.62%":>12} {"99.71%":>10}')
print(f'{"MS-TCANet paper":<22} {"-":>8} {"74.67%+/-1.24%":>16} {"96.36%":>12} {"~100%":>10}')
print(sep)
print(f'Delta vs MS-TCANet: {np.mean(r10)*100 - 74.67:+.2f}%')
print(f'Delta vs Lite:     {np.mean(r10)*100 - 69.03:+.2f}%')
sys.stdout.flush()
